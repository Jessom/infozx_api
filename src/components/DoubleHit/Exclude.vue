<template>
  <side-panel :options='{id: "z-exclude-wrap", style: "bottom", height: "80vh"}'>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.select = []
      this.staff = []
      clearSelect(mui('#z-exclude-wrap .z-select-wrap .mui-selected'))
    },
    // 进入下一级部门
    onCell(e, item) {
      e.stopPropagation()
    ...
    close() {
      this.init()
      mui('#z-exclude-wrap').popover('toggle')
    }
  },
  props: {
    list: Array
