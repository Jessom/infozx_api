		...
		<div class="mui-pull-left z-staff-right">
			<div class="mui-scroll-wrapper z-hentai-scroll">
				<div class="mui-scroll">
					...
					<div class="mui-input-group z-include-group">
						<div
							...
							<input name="checkbox" :disabled='select || item.select' :checked='item.select' value="Item 1" type="checkbox"  @tap='inputclick($event, item)' >
						</div>
					</div>
				</div>
			</div>
	...
	methods: {
		...
		clearCheckBox() {
			...
			if(this.actived.length > 0) {
				mui('.z-include-group input[type="checkbox"]').each((ind, item) => {
					item.checked = false
				})				
			}
		},
